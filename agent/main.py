...
from agent.api.batch_images import router as batch_images_router
...
# Extension WebSocket is now handled by FastAPI /ws endpoint
...
app.include_router(batch_images_router, prefix="/api")
...
async def extension_ws(websocket: WebSocket):
    """FastAPI WebSocket endpoint for the Chrome extension."""
    await websocket.accept()
    
    client = get_flow_client()
    # We need a wrapper to mimic the 'websockets' library interface used by flow_client
    class WSWrapper:
        def __init__(self, ws):
            self.ws = ws
            self.remote_address = ws.client.host if ws.client else "unknown"
    ...
    client.set_extension(wrapper)
    logger.info("Extension connected via FastAPI /ws from %s", wrapper.remote_address)
