...
async def _process_one(req: dict, deferred: dict = None, retry_after: dict = None):
    ...
    try:
        ...
        if _is_error(result):
            ...
            if gen_result.url:
                from agent.services.asset_downloader import auto_download_asset
                asyncio.create_task(auto_download_asset(req, gen_result.url))
...
async def _handle_failure(rid: str, req: dict, result: dict, retry_after: dict = None):
    ...
    # Rate limit (429) errors: wait much longer before retry (starting at 2 min)
    is_rate_limit = (result.get("status") == 429) or ("too many requests" in error_lower)
    if is_rate_limit:
        retry = req.get("retry_count", 0) + 1
        if retry < MAX_RETRIES:
            now = time.time()
            wait_time = min(2 ** retry * 60, 900)  # 2m, 4m, 8m, max 15m
            if retry_after is not None:
                retry_after[rid] = now + wait_time
            await crud.update_request(rid, status="PENDING", retry_count=retry, error_message=f"rate_limit: {error_msg}")
            logger.warning("Request %s hit rate limit (429), will retry in %ds: %s", rid[:8], int(wait_time), error_msg)
            return
