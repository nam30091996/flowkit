...
@router.get("/{pid}/batch-data")
async def get_batch_data(pid: str):
    """Load persisted batch items (prompts, image URLs, etc.) for a project."""
    repo = _get_repo()
    project = await repo.get_project(pid)
    if not project:
        raise HTTPException(404, "Project not found")
    ...
    project_name = slugify(getattr(project, "name", "project"))
    data_path = BASE_DIR / "output" / project_name / "batch_data.json"
    ...
    try:
        ...
        logger.error("Failed to load batch data for %s: %s", pid, e)
        return {"items": []}


@router.post("/{pid}/batch-data")
async def save_batch_data(pid: str, body: dict):
    """Persist batch items (prompts, image URLs, etc.) for a project."""
    repo = _get_repo()
    project = await repo.get_project(pid)
    if not project:
        raise HTTPException(404, "Project not found")
    ...
    out_dir.mkdir(parents=True, exist_ok=True)
    data_path = out_dir / "batch_data.json"
    
    try:
        data_path.write_text(json.dumps(body, indent=2, ensure_ascii=False), encoding="utf-8")
        return {"success": True}
    except Exception as e:
        logger.error("Failed to save batch data for %s: %s", pid, e)
        raise HTTPException(500, f"Failed to save batch data: {e}")
